...
export class RealTimeAPIService {
    ...
    async syncFDAData() {
        try {
            ...
            const fdaEndpoints = this.apiEndpoints.filter(ep => ep.name.includes('FDA') && ep.priority === 'high');
            const results = await Promise.allSettled(fdaEndpoints.map(endpoint => this.fetchFromAPI(endpoint)));
            let totalRecords = 0;
    ...
    async syncClinicalTrialsData() {
        try {
            ...
            const clinicalEndpoint = this.apiEndpoints.find(ep => ep.name === 'Clinical Trials Medical Devices');
            if (!clinicalEndpoint) {
    ...
    async syncEUData() {
        try {
            ...
            const euEndpoints = this.apiEndpoints.filter(ep => (ep.name.includes('EMA') || ep.name.includes('BfArM') ||
            ...
            const results = await Promise.allSettled(euEndpoints.map(endpoint => this.fetchFromAPI(endpoint)));
            let totalRecords = 0;
            let successfulSyncs = 0;
            const syncSummary = {};
            for (let i = 0; i < results.length; i++) {
...
**Authority:** ${authority}
    ...
    transformFDARecord(record, endpoint) {
        const hasValidData = record && (record.device_name ||
...
This clinical trial involves medical devices and is relevant for regulatory intelligence monitoring.`;
    ...
    determineClinicalTrialPriority(trial) {
        ...
        if (phase.includes('phase 3') || phase.includes('phase iii'))
            return 'high';
        if (status.includes('completed') && (phase.includes('phase 2') || phase.includes('phase ii')))
            return 'medium';
        return 'low';
    }
    async checkForDuplicate(update) {
        try {
            ...
            return allUpdates.some(existing => existing.id === update.id ||
...
//# sourceMappingURL=realTimeAPIService.js.map