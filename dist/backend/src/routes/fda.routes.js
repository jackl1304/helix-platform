...
router.get('/stats', async (req, res) => {
    try {
        ...
        const complianceScore = Math.max(0, Math.min(100, 100 - (activeRecalls * 5) - (criticalRecalls * 10) - (mockFDAEvents.length * 2)));
        res.json({
            success: true,
            data: {
                totalApprovals: mockFDAApprovals.length,
...
router.post('/sync-510k', async (req, res) => {
    try {
...
router.post('/sync-recalls', async (req, res) => {
    try {
...
//# sourceMappingURL=fda.routes.js.map