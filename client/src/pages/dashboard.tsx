...
export default function Dashboard() {
  ...
  // Optimierte Dashboard-Cards mit 3D-Navigation und Quellenlinks
  const dashboardCards = [
    {
      ...
      clickPath: '/regulatory-updates',
      sourceUrl: '/api/regulatory-updates',
      sourceName: 'Regulatory Intelligence API'
    },
    {
      ...
      description: 'Rechtsprechung und Urteile',
      quality: 'Vollständig',
      clickPath: '/rechtsprechung-fixed',
      sourceUrl: '/api/legal-cases',
      sourceName: 'Legal Cases Database'
    },
    {
      title: 'FDA Daten',
      value: '101',
    ...
    },
    {
      ...
      description: 'Global regulatory data sources',
      quality: 'Active',
      clickPath: '/global-sources',
      sourceUrl: '/api/data-sources',
      sourceName: 'Data Sources API'
    },
    {
      title: 'AI Insights',
      value: '24/7',
      ...
      description: 'KI-gestützte Regulatory Intelligence',
      quality: 'Premium',
      clickPath: '/ai-insights',
      sourceUrl: '/api/ai-analysis',
      sourceName: 'Gemini AI Service'
    },
    {
      title: 'Zulassungen',
      ...
      sourceUrl: '/api/approvals',
      sourceName: 'Approvals Tracking System'
    }
  ];
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center space-x-4 mt-4">
                ...
                <Badge variant="secondary" className="bg-white/20 text-white">
                  ...
                  {t('dashboard.activeSourcesShort')}
                </Badge>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="text-center">
                ...
                <div className="text-blue-200 text-sm">{t('dashboard.updates')}</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-white">100%</div>
                <div className="text-blue-200 text-sm">{t('dashboard.quality')}</div>
              </div>
            </div>
          </div>
        </div>

        {/* Stats Grid - 3D Klickbare Fenster mit direkter Navigation */}
          ...
          {dashboardCards.map((card, index) => {
            ...
            return (
              <Card 
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow = '0 25px 50px rgba(59, 130, 246, 0.5), 0 15px 30px rgba(59, 130, 246, 0.4), 0 8px 16px rgba(0, 0, 0, 0.15), 0 4px 8px rgba(59, 130, 246, 0.6)';
                  e.currentTarget.style.transform = 'perspective(1000px) translateY(-12px) translateZ(40px) scale(1.04)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.boxShadow = '0 8px 25px rgba(59, 130, 246, 0.2), 0 4px 10px rgba(0, 0, 0, 0.1), 0 2px 4px rgba(59, 130, 246, 0.3)';
                  e.currentTarget.style.transform = 'perspective(1000px) translateZ(0)';
                }}
                onClick={() => card.clickPath && setLocation(card.clickPath)}
                data-testid={`card-${card.title.toLowerCase().replace(/\s+/g, '-')}`}
              >
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                  <CardTitle className="text-sm font-medium text-gray-600 dark:text-gray-400 group-hover:text-blue-600 transition-colors">
                    {card.title}
                  </CardTitle>
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-gray-900 dark:text-white mb-2 group-hover:text-blue-700 transition-colors">
                    {typeof card.value === 'string' ? card.value : Number(card.value).toLocaleString()}
                  </div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 group-hover:text-gray-700 transition-colors">
                    {card.description}
                  </p>
                  {card.quality && (
                    <div className="mt-3">
                      <Badge variant="secondary" className="text-xs bg-green-100 text-green-800 group-hover:bg-green-200 transition-colors">
                  ...
                  {card.sourceUrl && (
                    <div className="mt-2 pt-2 border-t border-gray-100">
                      <div className="flex items-center justify-between text-xs text-gray-400">
                        <span>Quelle: {card.sourceName}</span>
                        <span className="text-blue-500 group-hover:text-blue-700">→ Klicken für Details</span>
                      </div>
                    </div>
                  )}
                </CardContent>
          ...
          <Card className="shadow-md">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2 text-xl">
                ...
                <span>{t('dashboard.regulatoryUpdates')}</span>
              </CardTitle>
              <CardDescription>
                {t('dashboard.regulatoryDescription')}
              </CardDescription>
            </CardHeader>
          ...
          <Card className="shadow-md">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2 text-xl">
                ...
                <span>{t('dashboard.newsletterSources')}</span>
              </CardTitle>
              <CardDescription>
                {t('dashboard.newsletterDescription')}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
