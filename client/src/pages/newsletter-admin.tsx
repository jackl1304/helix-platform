...
export default function NewsletterAdminPage() {
  ...
  const saveSourceMutation = useMutation({
    ...
    onError: () => {
      toast({
        ...
        description: t('newsletter.sourceSaveError'),
        variant: "destructive"
      });
    }
  });
  ...
  const testSourceMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({
        title: t('newsletter.testSuccess'),
        description: `${t('newsletter.connectionSuccess')} ${data.articlesFound || 0} ${t('newsletter.articlesFound')}.`,
      });
    },
    onError: () => {
      toast({
        title: t('newsletter.testFailed'),
        description: t('newsletter.connectionFailed'),
        variant: "destructive"
      });
    }
  });
  ...
  const handleSave = () => {
    if (!formData.name || !formData.url) {
      toast({
        title: t('common.error'),
        description: t('newsletter.nameUrlRequired'),
        variant: "destructive"
      });
      return;
    }
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-[#d95d2c] hover:bg-[#b8441f]">
              ...
              {t('newsletter.addSource')}
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>{t('newsletter.configureSource')}</DialogTitle>
              <DialogDescription>
                {t('newsletter.configureSourceDesc')}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t('newsletter.selectCategory')} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="industry_newsletter">{t('newsletter.industryNewsletter')}</SelectItem>
                      <SelectItem value="regulatory_newsletter">{t('newsletter.regulatoryNewsletter')}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div>
                <Label htmlFor="url">{t('newsletter.websiteUrl')}</Label>
                <Input
                  id="url"
                  value={formData.url || ''}
                  onChange={(e) => setFormData(prev => ({ ...prev, url: e.target.value }))}
                  placeholder={t('newsletter.urlPlaceholder')}
                />
              </div>

              <div>
                <Label htmlFor="rssUrl">{t('newsletter.rssFeedUrl')}</Label>
                <Input
                  id="rssUrl"
                  value={formData.rssUrl || ''}
                  onChange={(e) => setFormData(prev => ({ ...prev, rssUrl: e.target.value }))}
                  placeholder={t('newsletter.rssPlaceholder')}
                />
              </div>

              <div>
                <Label htmlFor="description">{t('newsletter.description')}</Label>
                <Textarea
                  id="description"
                  value={formData.description || ''}
                  onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))}
                  placeholder={t('newsletter.descriptionPlaceholder')}
                />
              </div>
