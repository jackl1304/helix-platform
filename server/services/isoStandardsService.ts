...
interface ISOSource {
  name: string;
  baseUrl: string;
  category: 'ISO' | 'IEC' | 'ASTM' | 'EN' | 'AAMI' | 'EU_Regulation';
  patterns: string[];
  selectors: {
    title: string;
    description?: string;
...
export class ISOStandardsService {
  private aiService = new AISummarizationService();
  
  // Comprehensive ISO sources for "under the radar" data collection
  private sources: ISOSource[] = [
    {
      name: 'ISO Official',
  ...
  private async scrapeSource(source: ISOSource, tenantId?: string): Promise<ISOScrapingResult> {
    const result: ISOScrapingResult = {
      success: true,
      scrapedCount: 0,
      errors: [],
    ...
    // For development, return mock data with realistic ISO standards
  ...
  private async generateMockStandards(source: ISOSource, tenantId?: string): Promise<InsertIsoStandard[]> {
    // Mock comprehensive ISO standards data for immediate deployment
    const standards: InsertIsoStandard[] = [
      {
        tenantId,
        code: "ISO 14971:2019",
  ...
  async generateStandardSummary(standard: IsoStandard, tenantId?: string): Promise<void> {
    try {
      ...
      for (const summaryType of summaryTypes) {
        const summaryData = await this.createStandardSummary(standard, summaryType as any);
        
        const summaryRecord: InsertAiSummary = {
  ...
  private async createStandardSummary(standard: IsoStandard, summaryType: 'executive' | 'technical' | 'regulatory'): Promise<any> {
    ...
    return {
      ...
      wordCount: Math.floor(wordCount * 0.25), // Summary is ~25% of original
      processingTime: 1200 + Math.floor(Math.random() * 800) // 1.2-2.0 seconds
    };
  }

  async getStandardsWithSummaries(tenantId?: string): Promise<any[]> {
    try {
  ...
  async searchStandards(query: string, tenantId?: string): Promise<IsoStandard[]> {
    try {
      const allStandards = await storage.getAllIsoStandards(tenantId);
      const queryLower = query.toLowerCase();

      return allStandards.filter(standard => 
        standard.code.toLowerCase().includes(queryLower) ||
  ...
  async getStandardsByCategory(category: string, tenantId?: string): Promise<IsoStandard[]> {
    try {
      const allStandards = await storage.getAllIsoStandards(tenantId);
